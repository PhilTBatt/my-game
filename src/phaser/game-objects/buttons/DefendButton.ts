...
export default class DefendButton extends Button {
    ...
    constructor(scene: BattleScreen, x: number, y: number, defend: Action) {
        ...
        const valueText = scene.add.text(nameText.width / 2 - 70, 0, `${defend.value}`, {fontSize: '60px', color: '#000000', fontFamily: 'Arial'})
        valueText.setOrigin(0.5)
        this.blockIcon = scene.add.image(nameText.width / 2 - 20, 0, 'damage-icon').setScale(0.25)
